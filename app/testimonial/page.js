...
export default function SubmitTestimonial() {
  ...
  const [form, setForm] = useState({ name: "", neighborhood: "", quote: "" });
  ...
  async function handleSubmit() {
    if (!form.name || !form.quote) {
      setError("Please enter your name and a quote.");
      return;
    }
    setSaving(true);
    setError("");
  ...
  return (
    <div className="min-h-screen bg-stone-50 text-gray-900" style={{fontFamily: "'Georgia', serif"}}>
      ...
      <div className="max-w-lg mx-auto px-4 sm:px-8 pt-16 pb-16">
        {success ? (
          <div className="text-center">
            <div className="text-5xl mb-6">🌟</div>
            <h1 className="text-3xl font-bold text-stone-900 mb-4">Thank you!</h1>
            <p className="text-stone-500 text-base leading-relaxed mb-8" style={{fontFamily: "sans-serif"}}>Your testimonial has been submitted. Aimee will review it and publish it shortly.</p>
            <a href="/" className="text-sm font-semibold" style={{color: "#c2446e", fontFamily: "sans-serif"}}>← Back to home</a>
          </div>
        ) : (
          <>
            <div className="text-xs tracking-widest uppercase font-semibold mb-3" style={{color: "#c2446e", fontFamily: "sans-serif"}}>Share Your Experience</div>
            <h1 className="text-3xl sm:text-4xl font-bold text-stone-900 mb-4" style={{letterSpacing: "-0.5px"}}>Leave a testimonial for Aimee.</h1>
            <p className="text-stone-500 text-base leading-relaxed mb-10" style={{fontFamily: "sans-serif"}}>Worked with Aimee? We&apos;d love to hear about your experience. Your words help other New Yorkers find the right broker.</p>

            <div className="bg-white rounded-xl border border-stone-200 p-6 space-y-4" style={{fontFamily: "sans-serif"}}>
              {[
                { label: "Your Name", key: "name", placeholder: "Jane Smith", type: "input" },
                { label: "Neighborhood", key: "neighborhood", placeholder: "Upper West Side", type: "input" },
              ].map(field => (
                <div key={field.key}>
              ...
              <div>
                <label className="block text-xs font-semibold text-stone-500 uppercase tracking-widest mb-1">Your Testimonial</label>
                <textarea
                  placeholder="Tell us about your experience working with Aimee..."
                  value={form.quote}
              ...
              <p className="text-stone-400 text-xs text-center">Testimonials are reviewed before publishing.</p>
            </div>
          </>
        )}
      </div>
